...
Example dual45 : Dual proto4 proto5. unfold Dual. simpl. repeat (split; trivial).
...
Example dual67 : Dual proto6 proto7. unfold Dual. simpl. repeat (split; trivial).
...
| OfferC {r s : protoType} {R S:type}
  : (protoExp R r) -> (protoExp S s) -> (protoExp (Either R S) (Offer r s))
| DecryptC {p':protoType} {mt T:type} : (message (Encrypt mt)) -> keyType -> ((message mt) -> protoExp T p') -> protoExp T p'(*(Decrypt p')*)
| ReturnC {t:type} : (message t) -> protoExp t (Eps t).
...
Fixpoint DualT_dec (t t':protoType) : {DualT t t'} + {~ DualT t t'}. destruct t. destruct t'. apply right. simpl. unfold not. trivial. assert ({t = t1} + {t <> t1}). apply (eq_type_dec t t1). assert ({DualT t0 t'} + {~ DualT t0 t'}). apply (DualT_dec t0 t'). destruct H. destruct H0. apply left. unfold DualT. simpl. split; assumption. fold DualT; try (apply right; simpl; unfold not; intros; destruct H; contradiction). try (right; unfold not; intros; inversion H). contradiction. right. unfold not. intros. inversion H. right. unfold not. intros. inversion H.

                                                                     right. unfold not. intros. inversion H.  destruct t'. assert ({t = t1} + {t <> t1}). apply (eq_type_dec t t1). assert ({DualT t0 t'} + {~ DualT t0 t'}). apply (DualT_dec t0 t'). destruct H. destruct H0. apply left. simpl. split. assumption. unfold DualT in d. assumption. right. unfold not. intros. destruct H. contradiction. apply right. unfold not. intros. destruct H. contradiction. right. unfold not. intros. inversion H. apply right. unfold not. intros. inversion H. apply right. unfold not. intros. inversion H. apply right. unfold not. intros. inversion H. destruct t'. apply right. unfold not. intros. inversion H. apply right. unfold not. intros. inversion H. apply right. unfold not. intros. inversion H. assert ({DualT t1 t'1 (* (max (protoTypeLength t2) (protoTypeLength t'2))*)} + {~ DualT t1 t'1 (*(max (protoTypeLength t2) (protoTypeLength t'2))*)}). apply (DualT_dec t1 t'1). assert ({DualT t2 t'2} + {~ DualT t2 t'2}). apply (DualT_dec t2 t'2). destruct H. destruct H0. left. unfold DualT. simpl. split. unfold DualT in d.  unfold DualT in d0. assumption. assumption. right. unfold not. intros. destruct H. contradiction. right. unfold not. intros. destruct H. contradiction. right. unfold not. intros. inversion H. destruct t'. right. unfold not. intros. inversion H.  right. unfold not. intros. inversion H. assert ({DualT t1 t'1} + {~ DualT t1 t'1}). apply (DualT_dec t1 t'1). assert ({DualT t2 t'2} + {~ DualT t2 t'2}). apply (DualT_dec t2 t'2). destruct H. destruct H0. left. simpl. split; assumption. right. unfold not. intros. destruct H. contradiction. right. unfold not. intros. destruct H. contradiction. right. unfold not. intros. inversion H. right. unfold not. intros. inversion H. destruct t'. right. unfold not. intros. inversion H. right. unfold not. intros. inversion H. right. unfold not. intros. inversion H. right. unfold not. intros. inversion H. left. simpl. trivial. Defined.
...
(Receive (Encrypt Basic)
  (Send (Encrypt (Pair Basic Basic)) (*(Decrypt ( *)
    (Receive (Encrypt Basic) (Eps (Encrypt Basic)))))(*))*). simpl; try (split; trivial). split; trivial. split; trivial. Qed.

Example DualNeedham : Dual Needham_A Needham_B.
Proof. unfold Dual. split. trivial. split. trivial. split. trivial. simpl. trivial. Qed.
...
Fixpoint runProto' {t t':protoType} {T T':type} (p1:protoExp T t) (p2:protoExp T' t') (l: nat)
  : (message T) + {(protoError p1 p2)}. (*{(~ (Dual p1 p2))}*)
                    case_eq p1. case_eq p2; intros. apply inright. apply NotDual. unfold not. trivial. assert ({t0 = t1} + {t0 <> t1}). apply (eq_type_dec t0 t1). destruct H1. subst. clear H. clear H0. assert (message T1 + {(protoError p0 (p m))}). apply (runProto' p'0 p' T1 T0 p0 (p m) (protoTypeLength p'0)). destruct X. apply inleft. exact m0. apply inright. destruct p3. apply NotDual. unfold not. intros. inversion H0. contradiction. apply NoDecrypt. clear H. clear H0. apply inright. apply NotDual. unfold not. intros. inversion H. symmetry in H0. contradiction. apply inright. apply NotDual. unfold not. intros. inversion H1. intros. apply inright. apply NotDual. unfold not. intros. inversion H1. (*assert ({T0=T1}+{T0<>T1}). apply (eq_type_dec T0 T1). destruct H1. subst.*) destruct (decrypt m k).
                    assert (message T1 + {(protoError (SendC m0 p0) (p m1))}). apply (runProto' _ _ _ _ (SendC m0 p0) (p m1) (protoTypeLength p'0)). destruct X. apply inleft. exact m2. clear H0. clear H. assert ({DualT (Send t0 p'0) p'}+{~DualT (Send t0 p'0) p'}). apply (DualT_dec (Send t0 p'0) p'). destruct H. apply inright. apply NoDecrypt. apply inright. apply NotDual.
                    unfold not. intros. unfold Dual in H. contradiction. apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. contradiction. intros. destruct p2. assert ({t0 = t1} + {t0 <> t1}). apply (eq_type_dec t0 t1). destruct H0. subst. clear H. assert (message T0 + {(protoError (p m) p2)}). apply (runProto' _ _ _ _ (p m) p2 (protoTypeLength p')). destruct X. apply inleft. exact m0. apply inright. destruct p0. apply NotDual. unfold not. intros. inversion H0. contradiction. apply NoDecrypt. clear H. apply inright. apply NotDual. unfold not. intros. inversion H. contradiction. apply inright. apply NotDual. unfold not. intros. inversion H0. apply inright. apply NotDual. unfold not. intros. inversion H0. apply inright. apply NotDual. unfold not. intros. inversion H0. assert ({(DualT (Receive t0 p') p'0)} + {~(DualT (Receive t0 p') p'0)}). apply (DualT_dec (Receive t0 p') p'0). destruct H0. destruct (decrypt m k). assert ({DualT (Receive t0 p') p'0}+{~DualT (Receive t0 p') p'0}). apply (DualT_dec (Receive t0 p') p'0). destruct H0. apply p0 in m0.

                    assert (message T0 + {(protoError (ReceiveC p) m0)}). apply (runProto' _ _ _ _ (ReceiveC p) m0 (protoTypeLength p'0)). destruct X. apply inleft. exact m1. apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. unfold Dual in H0. contradiction. apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. unfold Dual in H0. contradiction. apply inright. apply NotDual. unfold not. intros. unfold Dual in H0. contradiction. intros.

                    (*assert ({DualT (Send t0 p'0) p'}+{~DualT (Send t0 p'0) p'}). apply (DualT_dec (Send t0 p'0) p'). destruct H. apply inright. apply NoDecrypt. apply inright. apply NotDual. 
                    
n
                    assert ({T0=T1}+{T0<>T1}). apply (eq_type_dec T0 T1). simpl in d. destruct p'0. destruct d. subst. apply p0 in m0. destruct m0. assert ({t0 = t1} + {t0<>t1}). apply (eq_type_dec t0 t1). destruct H1. subst.
                    assert (message T0 + {(protoError (p m0) m1)}). apply (runProto' _ _ _ _  (p m0) m1 (protoTypeLength p'0)). destruct X. apply inleft. exact m2. clear H. subst. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. 

                    apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. contradiction.  apply inright. apply NotDual. unfold not. intros. contradiction. apply inright. apply NotDual. unfold not. intros. destruct b. destruct p2. apply inright. apply NotDual. unfold not. intros. inversion H0. apply inright. apply NotDual. unfold not. intros. inversion H0. apply inright. apply NotDual. unfold not. intros. inversion H0.
                    assert ({DualT r r0} + {~ DualT r r0}). apply (DualT_dec r r0). assert ({DualT s s0} + {~ DualT s s0}). apply (DualT_dec s s0). destruct H0. destruct H1. destruct (runProto' r r0 R R0 p p2_1 (protoTypeLength r0)). left. exact m. apply inright. destruct p2. contradiction. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. apply inright. apply NotDual. unfold not. intros. inversion H0.  apply inright. apply NotDual. unfold not. intros. inversion H0. destruct p2. apply inright. apply NotDual. unfold not. intros. inversion H0. apply inright. apply NotDual. unfold not. intros. inversion H0. apply inright. apply NotDual. unfold not. intros. inversion H0.
                    ...
                    unfold Dual. assert ({DualT s s0} + {~ DualT s s0}). apply (DualT_dec s s0). assert ({DualT r r0} + {~ DualT r r0}). apply (DualT_dec r r0). destruct H0. destruct H1. destruct (runProto' s s0 S S0 p0 p2_2 (protoTypeLength s0)). left. exact m. unfold not in n. unfold Dual in n. apply n in d. inversion d. right. unfold not. intros. destruct H0. contradiction. right. unfold not. intros. destruct H0. contradiction. apply inright. intros. unfold not. intros. inversion H0. intros.
                                                                                                                                                                                                                                                                                                                                                                                                     ...
                                                                                                                                                                                                                                                                                                                                                                                                     intros. unfold Dual. simpl. unfold Dual in n. unfold not in n. unfold not.  intros. destruct H.  apply n in H0.  assumption. apply inright. unfold Dual. simpl. unfold not. unfold not in n. intros. destruct H1. symmetry in H1. apply n in H1.  assumption. intros. apply inright. unfold Dual. simpl. unfold not. trivial. intros. apply inright. unfold Dual. unfold not. trivial. intros. apply inright. unfold Dual. unfold not. trivial. destruct p2. intros. assert ({t0 = t1} + {t0 <> t1}). apply (eq_type_dec t0 t1). destruct H0. subst. assert (message T1 + {~ Dual (p m) p2 }). apply (runProto' p'0 p' T1 T0 (p m) p2 (protoTypeLength p'0)). destruct X. apply inleft. exact m0. apply inright. intros. unfold Dual. simpl. unfold Dual in n. unfold not in n. unfold not.  intros. destruct H0.  apply n in H1.  assumption. apply inright. unfold Dual. simpl. unfold not. unfold not in n. intros. destruct H0. symmetry in H0. apply n in H0.  assumption. intros. apply inright. unfold Dual. unfold not. trivial. intros.  apply inright. intros. unfold not. intros. inversion H0. intros. apply inright. unfold not. intros. inversion H0. intros. apply inright. unfold not. intros. inversion H0.  intros. destruct p2. destruct b. apply inright. unfold not. intros. inversion H0. apply inright. unfold not. intros. inversion H0. apply inright. unfold not. intros. inversion H0. apply inright. unfold not. intros. inversion H0.*) destruct b. destruct p2. apply inright. apply NotDual. unfold not. intros. contradiction. 
                    apply inright. apply NotDual. unfold not. intros. contradiction.
                    apply inright. apply NotDual. unfold not. intros. contradiction.
                    assert ({DualT r r0} + {~ DualT r r0}). apply (DualT_dec r r0). assert ({DualT s s0} + {~ DualT s s0}). apply (DualT_dec s s0). destruct H0. destruct H1. destruct (runProto' r r0 R R0 p p2_1 (protoTypeLength r0)). left. exact m. apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. destruct (decrypt m k). apply p2 in m0. destruct (runProto' _ _ _ _ p m0 (protoTypeLength p')). apply inleft. exact m1. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. contradiction. destruct p2. apply inright. apply NotDual. unfold not. intros. contradiction. apply inright. apply NotDual. unfold not. intros. contradiction. apply inright. apply NotDual. unfold not. intros. contradiction. assert ({DualT r r0} + {~ DualT r r0}). apply (DualT_dec r r0). assert ({DualT s s0} + {~ DualT s s0}). apply (DualT_dec s s0). destruct H0. destruct H1. destruct (runProto' _ _ _ _ p0 p (protoTypeLength r0)). left. exact m. apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. destruct (decrypt m k). apply p2 in m0. destruct (runProto' _ _ _ _ p0 m0 (protoTypeLength p')). apply inleft. exact m1. destruct (decrypt m k). apply p2 in m1. destruct (runProto' _ _ _ _ p0 m1 (protoTypeLength p')). apply inleft. exact m2. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. inversion H0. intros. destruct p2. apply inright. apply NotDual. unfold not. intros. inversion H0. apply inright. apply NotDual. unfold not. intros. inversion H0.

                    destruct b.
                    assert ({DualT r r0} + {~ DualT r r0}). apply (DualT_dec r r0). assert ({DualT s s0} + {~ DualT s s0}). apply (DualT_dec s s0). destruct H0. destruct H1. destruct (runProto' r r0 R R0 p p2_1 (protoTypeLength r0)). left. exact (leither _ _ m). apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction.

                    assert ({DualT r r0} + {~ DualT r r0}). apply (DualT_dec r r0). assert ({DualT s s0} + {~ DualT s s0}). apply (DualT_dec s s0). destruct H0. destruct H1. destruct (runProto' r r0 R R0 p p2_1 (protoTypeLength r0)). left. exact (leither _ _ m). apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. apply inright. apply NotDual. unfold not. intros. inversion H0. 

                    destruct (decrypt m k). apply p2 in m0. destruct (runProto' _ _ _ _ p m0 (protoTypeLength p')). apply inleft. exact (leither _ _ m1). apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. contradiction. intros. destruct (decrypt m k). destruct p2. assert (message T0 + {(protoError (p m0) (SendC m1 p2) )}). apply (runProto' _ _ _ _ (p m0) (SendC m1 p2) (protoTypeLength p'0)). destruct X. apply inleft. exact m2. apply inright. apply NoDecrypt. destruct (runProto' _ _ _ _ (p m0) (ReceiveC p0) (protoTypeLength p')). apply inleft. exact m1. apply inright. apply NoDecrypt. assert (message T0 + {(protoError (p m0) (ChoiceC b p2_1 p2_2) )}). apply (runProto' _ _ _ _ (p m0) (ChoiceC b p2_1 p2_2) (protoTypeLength p')). destruct X. apply inleft. exact m1. apply inright. apply NoDecrypt. assert (message T0 + {(protoError (p m0) (OfferC p2_1 p2_2) )}). apply (runProto' _ _ _ _ (p m0) (OfferC p2_1 p2_2) (protoTypeLength p')). destruct X. apply inleft. exact m1. apply inright. apply NoDecrypt. destruct (decrypt m1 k0). 

                    assert (message T0 + {(protoError (p m0) (p0 m2 ))}). apply (runProto' _ _ _ _ (p m0) (p0 m2) (protoTypeLength p')). destruct X. apply inleft. exact m3. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. assert (message T0 + {(protoError (p m0) (ReturnC m) )}). apply (runProto' _ _ _ _ (p m0) (ReturnC m) (protoTypeLength p')). destruct X. apply inleft. exact m2. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. intros. destruct p2. apply inright. apply NotDual. unfold not. intros. contradiction. apply inright. apply NotDual. unfold not. intros. contradiction. apply inright. apply NotDual. unfold not. intros. contradiction.  apply inright. apply NotDual. unfold not. intros. contradiction. destruct (decrypt m0 k). assert (message t0 + {(protoError (ReturnC m) (p m1 ))}). apply (runProto' _ _ _ _ (ReturnC m) (p m1) (protoTypeLength p')). destruct X. apply inleft. exact m2. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. apply inleft. exact m. Defined.

                    assert (message T0 + {(protoError (p m0) (p0 m2 ))}). apply (runProto' _ _ _ _ (p m0) (p0 m2) (protoTypeLength p')). destruct X. apply inleft. exact m3. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt.

                    contradiction. apply inright. apply NotDual. unfold not. intros. contradiction. apply inright. apply NotDual. unfold not. intros. contradiction. assert ({DualT r r0} + {~ DualT r r0}). apply (DualT_dec r r0). assert ({DualT s s0} + {~ DualT s s0}). apply (DualT_dec s s0). destruct H0. destruct H1. destruct (runProto' _ _ _ _ p0 p (protoTypeLength r0)). left. exact m. apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. apply inright. apply NotDual. unfold not. intros. inversion H0. contradiction. destruct (decrypt m k). apply p2 in m0. destruct (runProto' _ _ _ _ p0 m0 (protoTypeLength p')). apply inleft. exact m1. destruct (decrypt m k). apply p2 in m1. destruct (runProto' _ _ _ _ p0 m1 (protoTypeLength p')). apply inleft. exact m2. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. apply inright. apply NoDecrypt. apply inright. apply NotDual. unfold not. intros. inversion H0. intros. destruct p2. apply inright. apply NotDual. unfold not. intros. inversion H0. apply inright. apply NotDual. unfold not. intros. inversion H0.

                    
                    unfold not in n. unfold Dual in n. apply n in d. inversion d. right. unfold not. intros. destruct H0. contradiction. right. unfold not. intros. destruct H0. contradiction.
